...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $drugPrice = $_POST['drugPrice']; // Retrieve the drugPrice from the form
    ...
    // Retrieve the drugName and drugPrice from the drug table using the drugID
    $query = "SELECT drugName, drugPrice FROM drug WHERE drugID = ?";
    $stmt = mysqli_prepare($conn, $query);
    ...
    if (!$data) {
        echo "Error: Drug not found in the database.";
        exit;
    }
    ...
    $query = "INSERT INTO orderPharmacy (pharmcoID, pharmacistID, drugID, drugName, pharmacyID, quantity, drugWeight, drugPrice) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, 'iiisiiid', $pharmcoID, $pharmacistID, $drugID, $drugName, $pharmacyID, $quantity, $drugWeight, $drugPrice);
